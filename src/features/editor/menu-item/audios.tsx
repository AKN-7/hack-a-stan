  ...
  return (
    ...
    >
      <div
        ...
        className="flex cursor-pointer gap-4  py-1 text-sm hover:bg-accent"
      >
        <div className="flex h-12 items-center justify-center bg-secondary">
          <Music width={16} />
        </div>
        <div className="flex flex-col justify-center">
          <div>{audio.name}</div>
          <div className="text-muted-foreground">{audio.metadata?.author}</div>
          {audio.metadata?.mood && (
            <div className="text-xs text-muted-foreground">{audio.metadata.mood}</div>
          )}
        </div>
      </div>
    </Draggable>
