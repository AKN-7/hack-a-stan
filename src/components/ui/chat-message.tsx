...
// Keys to hide from the user-facing result display
const HIDDEN_RESULT_KEYS = ['elementId', 'insertedAt', 'clipId', 'wordIds', 'jobId'];

// Format tool result for display, hiding technical IDs
const formatResultForDisplay = (result: Record<string, any>): Record<string, any> => {
  const filtered: Record<string, any> = {};
  for (const [key, value] of Object.entries(result)) {
    if (HIDDEN_RESULT_KEYS.includes(key)) continue;
...
const ToolResult = ({ invocation }: { invocation: ToolResult }) => {
  ...
  const hasDetails = Object.keys(result).length > 1 || !message;

  // If there's just a message, show it simply
  ...
  // If there's a message with details, show message prominently with expandable details
      ...
      {hasDetails && (
        <Accordion type="single" collapsible className="group w-full">
          <AccordionItem value="tool-result" className="border-0">
            <AccordionTrigger className="flex items-center gap-2 text-xs text-muted-foreground/70 hover:text-foreground w-full p-0 py-1">
              <span>Details</span>
            </AccordionTrigger>
            <AccordionContent>
              <pre className="overflow-x-auto whitespace-pre-wrap text-foreground text-xs">
